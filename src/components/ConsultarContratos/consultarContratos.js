    ...
    methods:{
            ...
            mostrarContrato(contrato){
                   ...
                   this.mensaje= 'DATOS GENERALES'+
                                 '\n ----------------------------------------------------------------------------------------------------------------------------------------------------------------'+
                                 '\nHUD       :      C'+ contrato.IDContrato +
                                 '\nAsesor   :      '+ this.obtenerAsesor(contrato.DNI_Em)+
                                 '\nNumero de Sumnistro   :     '+contrato.numSum+
                                 '\nFecha de Contrato         :     '+contrato.Fecha_Con+
                                 '\n\nDATOS DEL CLIENTE'+
                                 '\n ----------------------------------------------------------------------------------------------------------------------------------------------------------------'+
                                 '\nDNI       :        '+contrato.DNI_cli+
                                 '\nNombre    :        '+this.obtenerCliente(contrato.DNI_cli)+
                                 '\n\nDATOS DEL DOMICILIO'+
                                 '\n ----------------------------------------------------------------------------------------------------------------------------------------------------------------'+
                                 '\nCódigo Domicilio       :        '+temporal[0]+
            ...
            obtenerDomicilio(number){
               var cliente= this.clientes.find((cliente) => cliente.DNI_cli=== number);
               if (cliente) {
                   var domicilio= this.domicilios.find((domicilio)=> domicilio.IDDomicilio===cliente.IDDomicilio)
                   if(domicilio){
            ...
            getAsesores(){
                ...
                .then((response)=>{
                    this.asesores= response.data.filter((asesor)=>asesor.IDCategoria===1);
                })
                .catch((error) =>
