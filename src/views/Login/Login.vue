						...
						<v-container class="login-container">
							<v-row>
								...
								<v-col
									...
									md="10"
									lg="10"
									class="d-flex align-center justify-end"></v-col>
							</v-row>
							<div>
								<v-row align="center" style="margin-top: -20%">
									<v-col cols="12" sm="5" md="10">
										<h2
											class="identificarse-1 alinear"
											style="margin-bottom: 0.5rem">
											Identificarse
											<h5
												class="bienvenido-1 alinearbien"
												style="margin-top: 0.3rem; margin-bottom: 0.1rem">
												Bienvenido
											</h5>
										</h2>
									</v-col>
									...
									<v-row align="center" style="margin-top: 5%">
										...
										<v-col cols="12">
											...
											<v-text-field
												...
												:clearable="false"
												dense></v-text-field>
											<p style="margin-bottom: 0.3rem; margin-top: 0%">
												Contraseña
											</p>
											<v-text-field
												label=""
...
<script>
	export default {
		...
		methods: {
			...
			obtenerCredenciales() {
				this.axios
					.get("http://localhost:3000/credenciales-empleado")
					.then((res) => {
			...
			verificar() {
				if (this.DNI_Em !== "") {
					const credencial = this.credenciales.some(
						(credencial) => credencial.DNI_Em !== this.DNI_Em
					);
					if (credencial) {
						(this.mensaje = "Verifique el DNI, ¡Ingrese nuevamente!"),
							(this.dialogError = true);
						this.typemsg = "error";
					}
					if (this.nombreusuario !== "" && this.contraseña !== "") {
						const credencial2 = this.credenciales.some(
							(credencial) =>
								credencial.nombreusuario !== this.nombreusuario &&
								credencial.contraseña !== this.contraseña
						);
						if (credencial2) {
							(this.mensaje =
								"Verifique el usuario y contraseña, ¡Ingrese nuevamente!"),
								(this.dialogError = true);
							this.typemsg = "error";
						}
					}
